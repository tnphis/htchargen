...
libspath = 'libs'
mainpath = os.path.dirname(os.path.abspath(__file__)).replace('\\','/') + '/app'
...
def minify_file(p_file):
	print mainpath
	resultfile = mainpath + '/js/' + p_file + '.js'
	resultminfile = mainpath + '/js/' + p_file + '.min.js'
	cmdstring = (jscompressorapp + ' --js ' + resultfile + ' --js_output_file ' + resultminfile)
	print cmdstring
	os.system(cmdstring)
