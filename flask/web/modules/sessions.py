...
class ObjectSessionManager(SessionManager):
	def __init__(self, secret, session_object):
		self.session_object = session_object
		self.secret = secret

	def exists(self, sid):
		if self.session_object.get(sid):
			return True
		else:
			return False
	...
	def remove(self, sid):
		print 'Removing session: %s' % sid
		self.session_object.pop(sid, None)

	def new_session(self):
		sid = _generate_sid()
		self.session_object['sid'] = {}
	...
	def get(self, sid, digest):
		...
		if self.session_object.get(sid) is None:
			print "Missing data?"
			return self.new_session()

		hmac_digest = self.session_object.get('hmac_digest')
		randval = self.session_object.get('randval')
	...
	def put(self, session):
		...
		self.session_object['hmac_digest'] = session.hmac_digest
		self.session_object['randval'] = session.randval
		self.session_object['data'] = dict(session)
