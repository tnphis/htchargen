var Character = Backbone.Model.extend({
	...
	url : function() {
		return '/app/api?get_character=true&character_id=' + this.id
	},
	...
	get_skill_cap_by_type: function(skill, type) {
		...
		if (type == 'primary') {
			...
			thresh = Global.settings.skill_attr_cap_theshold_primary
	...
	get_attr_requirements: function(skill) {
		if (skill.value == 6) {
			...
			var primary_req = 0
			if (next_value >= Global.settings.skill_attr_cap_theshold_primary) {
				primary_req = Number(Global.settings.skill_attr_cap_primary) + (next_value - Global.settings.skill_attr_cap_theshold_primary) * Global.settings.skill_attr_cap_per_lvl_primary
			}
			var secondary_req = 0
			...
			if (skill.name == 'Leadership') {
				console.log(next_value, primary_req, secondary_req)
			}
			if (return_txt == '') {
				return null
			} else {
